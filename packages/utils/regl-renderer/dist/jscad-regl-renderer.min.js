...
const{flatten:flatten,toArray:toArray}=require("@jscad/array-utils"),{meshColor:meshColor}=require("../rendering/renderDefaults"),geom2ToGeometries=require("./geom2ToGeometries"),geom3ToGeometries=require("./geom3ToGeometries"),path2ToGeometries=require("./path2ToGeometries"),assembleEntities=e=>{return e.map(e=>{return{geometry:e,visuals:{drawCmd:"drawMesh",show:!0,transparent:e.isTransparent,useVertexColors:!0}}})},entitiesFromSolids=(e,...o)=>{const r={color:meshColor,smoothNormals:!0},{color:t,smoothNormals:s}=Object.assign({},r,e);o=(o=flatten(toArray(o))).filter(e=>e&&e instanceof Object);const i=[];return o.forEach(e=>{let o=[];"sides"in e?o=geom2ToGeometries({color:t},e):"points"in e?o=path2ToGeometries({color:t},e):"polygons"in e&&(o=geom3ToGeometries({smoothLighting:s,normalThreshold:.3,color:t},e)),i.push(...assembleEntities(o))}),i};module.exports=entitiesFromSolids;

},{"../rendering/renderDefaults":108,"./geom2ToGeometries":95,"./geom3ToGeometries":96,"./path2ToGeometries":97,"@jscad/array-utils":4}],95:[function(require,module,exports){
const mat4=require("gl-mat4"),maxIndex=Math.floor(32767.5),geom2ToGeometries=(o,e)=>{let{color:t}=o;"color"in e&&(t=e.color);const r=t[3]<1,s=e.sides,n=Math.floor(s.length/maxIndex)+1,a=[];for(let o=0;o<n;o++){const n=o*maxIndex,m=Math.min(n+maxIndex,s.length),l=[];for(let o=n;o<m;o++){const e=s[o];l.push([e[0][0],e[0][1],0]),l.push([e[1][0],e[1][1],0])}const c=l.map(o=>[0,0,-1]),i=l.map((o,e)=>e),p=e.transforms?mat4.clone(e.transforms):mat4.create();a.push({type:"2d",positions:l,normals:c,indices:i,transforms:p,color:t,isTransparent:r})}return a};module.exports=geom2ToGeometries;
...
const mat4=require("gl-mat4"),maxIndex=Math.floor(32767.5)-2,path2ToGeometries=(o,t)=>{let{color:s}=o;"color"in t&&(s=t.color);const e=s[3]<1,n=t.points,r=Math.floor(n.length/maxIndex)+1,a=[];for(let o=0;o<r;o++){const m=o*maxIndex,l=Math.min(m+maxIndex,n.length),p=[];let c;for(let o=m;o<l;o++){const t=n[o];c&&(p.push([c[0],c[1],0]),p.push([t[0],t[1],0])),c=t}if(o+1===r&&t.isClosed&&c){const o=n[0];p.push([c[0],c[1],0]),p.push([o[0],o[1],0])}const i=p.map(o=>[0,0,-1]),h=p.map((o,t)=>t),x=t.transforms?mat4.clone(t.transforms):mat4.create();a.push({type:"2d",positions:p,normals:i,indices:h,transforms:x,color:s,isTransparent:e})}return a};module.exports=path2ToGeometries;
...
const mat4=require("gl-mat4"),{meshColor:meshColor}=require("../../renderDefaults"),drawMesh=(e,r={extras:{}})=>{const{buffer:t}=e,o={useVertexColors:!0,dynamicCulling:!1,geometry:void 0,primitive:"triangles",color:meshColor};let{geometry:s,dynamicCulling:a,useVertexColors:l,primitive:i,color:n}=Object.assign({},o,r);const c=e.buffer([]),m=!!(s.indices&&s.indices.length>0),u=!!(s.normals&&s.normals.length>0),d=!!(l&&s.colors&&s.colors.length>0),g=a?(e,r)=>{return 1&[r.model[0],r.model[5],r.model[10]].filter(e=>e<0).length?"front":"back"}:"back",h=d?require("./vColorShaders").vert:require("./meshShaders").vert;let p=d?require("./vColorShaders").frag:require("./meshShaders").frag;"2d"===s.type&&(i="lines","color"in s&&(n=s.color),p=require("./colorOnlyShaders").frag);let f={vert:h,frag:p,uniforms:{model:(e,r)=>r.model||s.transforms||mat4.create(),ucolor:(e,r)=>r&&r.color?r.color:n,vColorToggler:(e,r)=>r&&r.useVertexColors&&!0===r.useVertexColors?1:0,unormal:(e,r)=>{const t=mat4.create(),o=mat4.multiply(mat4.create(),t,r.camera.view),s=mat4.create();return mat4.invert(s,o),mat4.transpose(s,s),s}},attributes:{position:t({usage:"static",type:"float",data:s.positions}),ao:c},cull:{enable:!0,face:g},depth:{enable:!0},blend:{enable:!0,func:{src:"src alpha",dst:"one minus src alpha"}},primitive:(e,r)=>r&&r.primitive?r.primitive:i};return s.cells?f.elements=s.cells:m?f.elements=e.elements({usage:"static",type:"uint16",data:s.indices}):s.triangles?f.elements=s.triangles:f.count=s.positions.length/3,u&&(f.attributes.normal=t({usage:"static",type:"float",data:s.normals})),d&&(f.attributes.color=t({usage:"static",type:"float",data:s.colors})),e(f=Object.assign({},f,r.extras))};module.exports=drawMesh;
