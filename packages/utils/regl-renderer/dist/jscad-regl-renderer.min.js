...
const vec3=require("gl-vec3"),boundingBox=require("./boundingBox"),computeBounds=e=>{let c,a,r,t,n,v;if(Array.isArray(e))e.forEach(e=>{let c=boundingBox(e.positions);c=c.map(c=>vec3.transformMat4(c,c,e.transforms)),r?(vec3.min(r[0],r[0],c[0]),vec3.max(r[1],r[1],c[1])):r=c}),n=vec3.min(vec3.create(),r[1],r[0]),v=vec3.max(vec3.create(),r[1],r[0]),t=vec3.subtract(vec3.create(),v,n),a=vec3.scale(vec3.create(),t,.5),a=vec3.add(a,n,a),c=vec3.distance(a,v);else{const o=e;r=(r=boundingBox(o.positions)).map(e=>vec3.transformMat4(e,e,o.transforms)),n=vec3.min(vec3.create(),r[1],r[0]),v=vec3.max(vec3.create(),r[1],r[0]),t=vec3.subtract(vec3.create(),v,n),a=vec3.scale(vec3.create(),t,.5),a=vec3.add(a,n,a),c=vec3.distance(a,v)}return{dia:c,center:[...a],min:[...n],max:[...v],size:[...t]}};module.exports=computeBounds;
...
const vec3=require("gl-vec3"),mat4=require("gl-mat4"),{max:max,min:min,sqrt:sqrt,PI:PI,sin:sin,cos:cos,atan2:atan2}=Math,computeBounds=require("../bound-utils/computeBounds"),controlsProps={limits:{minDistance:.01,maxDistance:1e4},drag:.27,EPS:1e-6,zoomToFit:{auto:!0,targets:"all",tightness:1.5},userControl:{zoom:!0,zoomSpeed:1,rotate:!0,rotateSpeed:1,pan:!0,panSpeed:1},autoRotate:{enabled:!1,speed:1},autoAdjustPlanes:!0},controlsState={thetaDelta:0,phiDelta:0,scale:1},defaults=Object.assign({},controlsState,controlsProps),update=({controls:t,camera:e},a)=>{const{EPS:o,drag:r}=t,{position:s,target:n}=e,c=t.up?t.up:e.up;let i=t.thetaDelta;const l=t.phiDelta,m=t.scale,p=vec3.subtract([],s,n);let u,d;1===c[2]?(u=atan2(p[0],p[1]),d=atan2(sqrt(p[0]*p[0]+p[1]*p[1]),p[2])):(u=atan2(p[0],p[2]),d=atan2(sqrt(p[0]*p[0]+p[2]*p[2]),p[1])),t.autoRotate.enabled&&t.userControl.rotate&&(i+=2*Math.PI/60/60*t.autoRotate.speed),u+=i,d=max(o,min(PI-o,d+=l));const h=max(t.limits.minDistance,min(t.limits.maxDistance,vec3.length(p)*m));1===c[2]?(p[0]=h*sin(d)*sin(u),p[2]=h*cos(d),p[1]=h*sin(d)*cos(u)):(p[0]=h*sin(d)*sin(u),p[1]=h*cos(d),p[2]=h*sin(d)*cos(u));const g=vec3.add(vec3.create(),n,p),v=mat4.lookAt(mat4.create(),g,n,c),D=1-max(min(r,1),.01);return{controls:{thetaDelta:i*D,phiDelta:l*D,scale:1,changed:vec3.distance(s,g)>0},camera:{position:g,view:v}}},rotate=({controls:t,camera:e,speed:a=1},o)=>{let{thetaDelta:r,phiDelta:s}=t;return t.userControl.rotate&&(r+=o[0]*a,s+=o[1]*a),{controls:{thetaDelta:r,phiDelta:s},camera:e}},zoom=({controls:t,camera:e,speed:a=1},o=0)=>{let{scale:r}=t;if(t.userControl.zoom&&e&&void 0!==o&&0!==o&&!isNaN(o)){const s=(o=o/o*(0===Math.sign(o)?1:Math.sign(o))*a)+t.scale,n=vec3.distance(e.position,e.target)*s;if(n>t.limits.minDistance&&n<t.limits.maxDistance&&(r+=o),"orthographic"===e.projectionType){const t=.3*vec3.length(vec3.subtract([],e.position,e.target)),a=Math.tan(e.fov)*t*e.aspect,o=Math.tan(e.fov)*t;e=require("../cameras/orthographicCamera").setProjection(e,{width:a,height:o})}}return{controls:{scale:r},camera:e}},pan=({controls:t,camera:e,speed:a=1},o)=>{const r=require("camera-unproject"),{projection:s,view:n,viewport:c}=e,i=mat4.multiply([],s,n),l=mat4.invert([],i),m=[c[2],c[3],0],p=[c[2]-o[0],c[3]+o[1],0],u=r([],m,c,l),d=r([],p,c,l),h=vec3.subtract([],u,d).map(e=>e*a*250*t.scale);return{controls:t,camera:{position:vec3.add(vec3.create(),e.position,h),target:vec3.add(vec3.create(),e.target,h)}}},zoomToFit=({controls:t,camera:e,entities:a})=>{const{zoomToFit:o}=t;if("all"!==o.targets)return{controls:t,camera:e};if(0===(a=a.filter(t=>t.type&&"3d"===t.type)).length)return{controls:t,camera:e};const r=a.map(t=>t.geometry),s=computeBounds(r),{fov:n,target:c,position:i}=e,{tightness:l}=Object.assign({},o,controlsProps.zoomToFit),m=s.dia*l/Math.tan(n/2)/vec3.distance(c,i);return{camera:{target:s.center},controls:{scale:m}}},reset=({controls:t,camera:e},a)=>{return{camera:{position:a.camera.position,target:a.camera.target,projection:mat4.perspective([],e.fov,e.aspect,e.near,e.far),view:a.camera.view},controls:{thetaDelta:a.controls.thetaDelta,phiDelta:a.controls.phiDelta,scale:a.controls.scale}}},setFocus=({controls:t,camera:e},a)=>{const o=(t,e)=>t.map((t,a)=>t+e[a]),r=e.target,s=[0,0,.5*((t,e)=>t.map((t,a)=>t-e[a]))(a,r)[2]];return e.target=o(r,s),e.position=o(e.position,s),e};module.exports={controlsProps:controlsProps,controlsState:controlsState,defaults:defaults,update:update,rotate:rotate,zoom:zoom,pan:pan,zoomToFit:zoomToFit,reset:reset};
...
const{flatten:flatten,toArray:toArray}=require("@jscad/array-utils"),computeBounds=require("../bound-utils/computeBounds"),{meshColor:meshColor}=require("../rendering/renderDefaults"),geom2ToGeometries=require("./geom2ToGeometries"),geom3ToGeometries=require("./geom3ToGeometries"),path2ToGeometries=require("./path2ToGeometries"),entitiesFromSolids=(e,o)=>{const r={color:meshColor,smoothNormals:!0},{color:t,smoothNormals:s}=Object.assign({},r,e);return(o=toArray(o)).map(e=>{let o,r;if(!e)return null;if(!(e instanceof Object))return null;if("sides"in e)r="2d",o=geom2ToGeometries({color:t},e);else if("points"in e)r="2d",o=path2ToGeometries({color:t},e);else{if(!("polygons"in e))return null;r="3d",o=geom3ToGeometries({smoothLighting:s,normalThreshold:.3,color:t},e)}const n=flatten(o)[0];if(!n)return null;const i=computeBounds(n);return i.dia<=0?null:{type:r,geometry:n,transforms:{matrix:n.transforms},bounds:i,visuals:{drawCmd:"drawMesh",show:!0,transparent:n.isTransparent,useVertexColors:!0}}}).filter(e=>null!==e)};module.exports=entitiesFromSolids;

},{"../bound-utils/computeBounds":82,"../rendering/renderDefaults":101,"./geom2ToGeometries":88,"./geom3ToGeometries":89,"./path2ToGeometries":90,"@jscad/array-utils":1}],88:[function(require,module,exports){
const mat4=require("gl-mat4"),geom2ToGeometries=(o,r)=>{let{color:s}=o;"color"in r&&(s=r.color);const e=s[3]<1,t=[];r.sides.forEach(o=>{t.push([o[0][0],o[0][1],0]),t.push([o[1][0],o[1][1],0])});const m=t.map(o=>[0,0,-1]),a=t.map((o,r)=>r),n=r.transforms?mat4.clone(r.transforms):mat4.create();return[{positions:t,normals:m,transforms:n,color:s,indices:a,isTransparent:e}]};module.exports=geom2ToGeometries;

},{"gl-mat4":19}],89:[function(require,module,exports){
const vec3=require("gl-vec3"),mat4=require("gl-mat4"),geom3ToGeometries=(o,e)=>{const{smoothLighting:t,normalThreshold:r,color:n}=o;return convert({color:n,smoothLighting:t,normalThreshold:r},e)},convert=(o,e)=>{let{color:t,smoothLighting:r,normalThreshold:n}=o;const s=[],l=[],i=[],c=[],a=[];"color"in e&&(t=e.color);let m=!1;const h=e.polygons,p=e.transforms?e.transforms:mat4.create();let u=[];u={};let g=0;for(let o=0;o<h.length;o++){const e=h[o],v=polygonColor(e,t),d=calculateNormal(e.vertices);v&&1!==v[3]&&(m=!0);const f=[];for(let o=0;o<e.vertices.length;o++){let t;const s=e.vertices[o],a=[s[0],s[1],s[2]];if(r){const o={normal:d,position:a},e=fuzyNormalAndPositionLookup(u,o,n);if(e)t=e.index;else{const e=u[o.position],r=[{normal:o.normal,index:g}];u[o.position]=e?u[o.position].concat(r):r,t=g,v&&i.push(v),c.push(d),l.push(a),g+=1}}else v&&i.push(v),c.push(d),l.push(a),t=l.length-1;f.push(t)}for(let o=2;o<f.length;o++)a.push([f[0],f[o-1],f[o]]);(l.length>65e3||o===h.length-1)&&(void 0===v?s.push({indices:a,positions:l,transforms:p,normals:c,color:t,isTransparent:m}):s.push({indices:a,positions:l,transforms:p,normals:c,colors:i,isTransparent:m}))}return s},polygonColor=(o,e)=>{let t=e;return o.color&&(t=o.color),t&&t.length<4&&t.push(1),t},calculateNormal=o=>{const e=vec3.create();vec3.subtract(e,o[1],o[0]);const t=vec3.create();vec3.subtract(t,o[2],o[0]);const r=vec3.create();return vec3.cross(r,e,t),vec3.normalize(r,r),r},areNormalsSimilar=(o,e,t)=>vec3.distance(o,e)<=t,fuzyNormalAndPositionLookup=(o,e,t)=>{const r=o[e.position];if(r)for(let o=0;o<r.length;o++){const n=r[o].normal;if(areNormalsSimilar(n,e.normal,t))return{tupple:{position:e.position,normal:n},index:r[o].index}}};module.exports=geom3ToGeometries;

},{"gl-mat4":19,"gl-vec3":52}],90:[function(require,module,exports){
const mat4=require("gl-mat4"),path2ToGeometries=(o,s)=>{let{color:t}=o;"color"in s&&(t=s.color);const r=t[3]<1,e=s.points,a=[];let n;if(e.forEach(o=>{n&&(a.push([n[0],n[1],0]),a.push([o[0],o[1],0])),n=o}),s.isClosed&&n){const o=e[0];a.push([n[0],n[1],0]),a.push([o[0],o[1],0])}const m=a.map(o=>[0,0,-1]),c=a.map((o,s)=>s),i=s.transforms?mat4.clone(s.transforms):mat4.create();return[{positions:a,normals:m,transforms:i,color:t,indices:c,isTransparent:r}]};module.exports=path2ToGeometries;

},{"gl-mat4":19}],91:[function(require,module,exports){
module.exports={prepareRender:require("./rendering/render"),drawCommands:{drawGrid:require("./rendering/commands/drawGrid/multi.js"),drawAxis:require("./rendering/commands/drawAxis"),drawMesh:require("./rendering/commands/drawMesh/index.js")},cameras:{camera:require("./cameras/camera"),orthographic:require("./cameras/orthographicCamera"),perspective:require("./cameras/perspectiveCamera")},controls:{orbit:require("./controls/orbitControls")},entitiesFromSolids:require("./geometry-utils-V2/entitiesFromSolids")};
...
const mat4=require("gl-mat4"),makeDrawGrid=(o,i={})=>{const n=[],t={visuals:{color:[1,1,1,1],fadeOut:!1},ticks:1,size:[16,16],centered:!1,lineWidth:2},e=Object.assign({},t.visuals,i.visuals||{}),{fadeOut:r,color:s}=e,{size:l,ticks:a,centered:c,lineWidth:d}=Object.assign({},t,i),f=l[0],u=l[1];if(c){const o=.5*f,i=.5*u,t=-o+o%a,e=-t,r=-i+i%a,s=-r,l=0;for(let o=t,i=0;o<=e;o+=a,i+=1)i%l!=0&&(n.push(r,o,0),n.push(s,o,0),n.push(r,o,0));for(let o=r,i=0;o<=s;o+=a,i+=1)i%l!=0&&(n.push(o,t,0),n.push(o,e,0),n.push(o,t,0))}else{for(let o=.5*-f;o<=.5*f;o+=a)n.push(.5*-u,o,0),n.push(.5*u,o,0),n.push(.5*-u,o,0);for(let o=.5*-u;o<=.5*u;o+=a)n.push(o,.5*-f,0),n.push(o,.5*f,0),n.push(o,.5*-f,0)}return o({vert:"precision mediump float;\n\n    uniform float camNear, camFar;\n    uniform mat4 model, view, projection;\n\n    attribute vec3 position;\n    varying vec3 fragNormal, fragPosition;\n    varying vec4 worldPosition;\n\n    void main() {\n      fragPosition = position;\n      worldPosition = model * vec4(position, 1);\n      vec4 glPosition = projection * view * worldPosition;\n      gl_Position = glPosition;\n    }",frag:"precision mediump float;\n    uniform vec4 color;\n    varying vec3 fragNormal, fragPosition;\n    varying vec4 worldPosition;\n\n    uniform vec4 fogColor;\n    uniform bool fadeOut;\n    void main() {\n      float dist = .5;\n      if(fadeOut){\n        dist = distance( vec2(0.,0.), vec2(worldPosition.x, worldPosition.y));\n        dist *= 0.0025;\n        dist = sqrt(dist);\n      }\n\n      gl_FragColor = mix(color, fogColor, dist);\n    }\n    ",attributes:{position:o.buffer(n)},count:n.length/3,uniforms:{model:(o,i)=>i&&i.model?i.model:mat4.identity([]),color:(o,i)=>i&&i.color?i.color:s,fogColor:(o,i)=>i&&i.color?[i.color[0],i.color[1],i.color[2],0]:[s[0],s[1],s[2],0],fadeOut:(o,i)=>i&&void 0!==i.fadeOut?i.fadeOut:r},lineWidth:(i,n)=>Math.min(n&&n.lineWidth?n.lineWidth:d,o.limits.lineWidthDims[1]),primitive:"lines",cull:{enable:!0,face:"front"},polygonOffset:{enable:!0,offset:{factor:1,units:10*Math.random()}},blend:{enable:!0,func:{src:"src alpha",dst:"one minus src alpha"}}})};module.exports=makeDrawGrid;

},{"gl-mat4":19}],94:[function(require,module,exports){
const makeDrawMultiGrid=(e,i)=>{const{size:r,ticks:t}=i,s=require("./index")(e,{size:r,ticks:t[0]}),u=require("./index")(e,{size:r,ticks:t[1]});return e=>{s(e),u({color:e.subColor,fadeOut:e.fadeOut})}};module.exports=makeDrawMultiGrid;

},{"./index":93}],95:[function(require,module,exports){
const vColorFrag="\nprecision mediump float;\nuniform vec4 ucolor;\n\nvoid main () {\n  gl_FragColor = ucolor;\n}\n";module.exports={frag:vColorFrag};

},{}],96:[function(require,module,exports){
const mat4=require("gl-mat4"),{meshColor:meshColor}=require("../../renderDefaults"),drawMesh=(e,r={extras:{}})=>{const{buffer:t}=e,o={useVertexColors:!0,dynamicCulling:!1,geometry:void 0,type:"3d",primitive:"triangles",color:meshColor};let{geometry:l,dynamicCulling:s,useVertexColors:i,type:a,primitive:n,color:m}=Object.assign({},o,r);const c=e.buffer([]),u=!!(l.indices&&l.indices.length>0),d=!!(l.normals&&l.normals.length>0),g=!!(i&&l.colors&&l.colors.length>0),h=s?(e,r)=>{return 1&[r.model[0],r.model[5],r.model[10]].filter(e=>e<0).length?"front":"back"}:"back",f=g?require("./vColorShaders").vert:require("./meshShaders").vert;let p=g?require("./vColorShaders").frag:require("./meshShaders").frag;"2d"===a&&(n="lines","color"in l&&(m=l.color),p=require("./colorOnlyShaders").frag);let v={vert:f,frag:p,uniforms:{model:(e,r)=>r.model||r.transforms.matrix||mat4.identity([]),ucolor:(e,r)=>r&&r.color?r.color:m,vColorToggler:(e,r)=>r&&r.useVertexColors&&!0===r.useVertexColors?1:0,unormal:(e,r)=>{const t=mat4.create(),o=mat4.multiply(mat4.create(),t,r.camera.view),l=mat4.create();return mat4.invert(l,o),mat4.transpose(l,l),l}},attributes:{position:t(l.positions),ao:c},cull:{enable:!0,face:h},depth:{enable:!0},blend:{enable:!0,func:{src:"src alpha",dst:"one minus src alpha"}},primitive:(e,r)=>r&&r.primitive?r.primitive:n};if(l.cells)v.elements=l.cells;else if(u){const r=l.indices;v.elements=e.elements({data:r})}else l.triangles?v.elements=l.triangles:v.count=l.positions.length/3;return d&&(v.attributes.normal=t(l.normals)),g&&(v.attributes.color=t(l.colors)),e(v=Object.assign({},v,r.extras))};module.exports=drawMesh;
...
const renderContext=require("./renderContext"),renderDefaults=require("./renderDefaults"),prepareRender=e=>{const r=Object.assign({},{},e.glOptions,{onDone:(e,r)=>{if(e)throw e}}),n=require("regl")(r),a=new Map;return function(e){var r;n.poll(),(r=e).rendering=Object.assign({},renderDefaults,r.rendering),renderContext(n)(r,e=>{n.clear({color:r.rendering.background,depth:1}),r.entities&&r.entities.sort((e,r)=>{const n="transparent"in e.visuals&&e.visuals.transparent;return n===("transparent"in r.visuals&&r.visuals.transparent)?0:n?1:-1}).forEach(e=>{const{visuals:t}=e;if(t.drawCmd&&t.show&&r.drawCommands[t.drawCmd]){let s;t.cacheId?s=a.get(t.cacheId):(t.cacheId=a.size,s=r.drawCommands[t.drawCmd](n,e),a.set(t.cacheId,s)),s({...e,...t,camera:r.camera})}})})}};module.exports=prepareRender;
