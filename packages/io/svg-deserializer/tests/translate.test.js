...
test('deserialize : translate svg (rect) to script', function (t) {
  ...
  let obs = deserializer.deserialize(sourceSvg, undefined, { output: 'script', target: '1D', addMetaData: false })
  ...
  obs = deserializer.deserialize(sourceSvg, undefined, { output: 'script', target: '2D', addMetaData: false })
  t.is(typeof obs, 'string')
  t.is(countOf('rectangle', obs), 1)
  t.is(countOf('roundedRectangle', obs), 3)
  t.is(countOf('color.color', obs), 3)
...
test('deserialize : translate svg (path: simple) to script', function (t) {
  ...
  t.is(countOf('path2.appendPoints', obs), 2)
  t.is(countOf('path2.close', obs), 1)
  t.is(countOf('color.color', obs), 0) // no fill
