...
import { Geom2, Path2 } from '../../geometries/types'
...
test('rotate: rotating of multiple objects produces expected changes', (t) => {
  ...
  const rotated = rotate([0, 0, Math.PI / 2], geometry1, geometry2)

  const obs1 = path2.toPoints(rotated[0] as Path2)
  const exp1 = [[-5, -5], [-5, 5], [5, -5], [5.000000000000001, 10]]
  t.true(comparePoints(obs1, exp1))

  const obs2 = geom2.toPoints(rotated[1] as Geom2)
