...
test('center: centering of multiple objects produces expected changes', (t) => {
  ...
  const centered = center({ axes: [true, true, false], relativeTo: [10, 15, 0] }, geometry1, geometry2)

  const pts1 = path2.toPoints(centered[0])
  const exp1 = [[2.5, 20], [12.5, 20], [2.5, 10], [17.5, 10]]
  t.true(comparePoints(pts1, exp1))

  const pts2 = geom2.toPoints(centered[1])
