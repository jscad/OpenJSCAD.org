...
test('square (defaults)', t => {
  ...
  t.deepEqual(obsSquare.sides.length, 4)
  t.truthy(comparePositonVertices(obsSquare.sides, expSides))
})

test('square (custom size, 2d array parameter)', t => {
  const obsSquare = square([2, 3])
...
test('square (custom size, size object parameter)', t => {
  const obsSquare = square({size: [2, 3]})
...
test('square (default size, centered)', t => {
  const obsSquare = square({center: true})
