...
// calculate offsets to position the paths into POSITIVE space
    var xoffset = 0 - bounds[0].x - ((bounds[1].x-bounds[0].x)/2) + ((bounds[1].x-bounds[0].x)/2);
    var yoffset = 0 - bounds[0].y - ((bounds[1].y-bounds[0].y)/2) + ((bounds[1].y-bounds[0].y)/2);
// FIXME should scaling be applied?
    var str = '<g transform="translate('+(0-xoffset)+','+(0-yoffset)+')">\n';
    paths.map(function(path) {
        str += '<path d="';
...
CAG.prototype.toSvg = function() {
    ...
    svg += '<svg version="1.1" baseProfile="tiny" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">\n';
