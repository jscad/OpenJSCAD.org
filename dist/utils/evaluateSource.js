...
function evaluateSource(modelingHelpers, CAG, mainParams, src) {
  ...
  */
  ...
  var mainFunction = 'main(_getParameterDefinitions(' + JSON.stringify(mainParams) + '))';
  var fullSrc = src + '\n' + modelingHelpers + '\n' + mainFunction + '\n';
  ...
  if (csgObject.length) {
    ...
    for (var i = 1; i < csgObject.length; i++) {
      var current = csgObject[i] instanceof CAG ? csgObject[i].extrude({ offset: [0, 0, 0.1] }) : csgObject[i];
