...
var _generateOutputData = require('./utils/generateOutputData');

var _generateOutputData2 = _interopRequireDefault(_generateOutputData);
...
var _parseArgs = parseArgs(args),
    inputFile = _parseArgs.inputFile,
    inputFormat = _parseArgs.inputFormat,
    outputFile = _parseArgs.outputFile,
    outputFormat = _parseArgs.outputFormat,
...
console.log('converting ' + inputFile + ' -> ' + outputFile + ' (' + _formats.formats[outputFormat].description + ')');

var src = _fs2.default.readFileSync(inputFile, inputFile.match(/\.stl$/i) ? 'binary' : 'UTF8');
...
(0, _generateOutputData2.default)(src, params, { outputFormat: outputFormat }).then(function (outputData) {
  writeOutputDataToFile(outputFile, outputData);
...
function parseArgs(args) {
  ...
  for (var i = 0; i < args.length; i++) {
    if (args[i] === '-of') {
      ...
      if (!_fs2.default.statSync(inputFile).isFile()) {
      ...
      (0, _env2.default)();
      console.log('OpenSCAD Compatibility (' + _version.version + ')');
    } else {
...
function determineOutputNameAndFormat(outputFormat, outputFile) {
  if (!outputFormat && outputFile && outputFile.length && outputFile.match(/\.(jscad|js|stl|amf|dxf|svg)$/)) {
    // output filename set
    ...
    process.exit(1);
  } else if (outputFormat.match(/(jscad|js|stl|stla|stlb|amf|dxf|svg)/i)) {
...
function writeOutputDataToFile(outputFile, outputData) {
  _fs2.default.writeFile(outputFile, outputData.asBuffer(), function (err) {
    if (err) {
      console.log('err', err);
    } else {
      //console.log('success')
