...
function mirror(v, o) {
   ...
   var plane = new _csg.CSG.Plane(new _csg.CSG.Vector3D(v[0], v[1], v[2]).unit(), 0);
...
function hull() {
   ...
   var ConvexHullPoint = function ConvexHullPoint(i, a, d) {
      ...
      this.compare = function (p) {
         if (this.angle < p.angle) return -1;else if (this.angle > p.angle) return 1;else {
            if (this.distance < p.distance) return -1;else if (this.distance > p.distance) return 1;
         }
         return 0;
      };
   };

   var ConvexHull = function ConvexHull() {
      this.points = null;
      this.indices = null;
      ...
      this.ccw = function (p1, p2, p3) {
         var ccw = (this.points[p2].x - this.points[p1].x) * (this.points[p3].y - this.points[p1].y) - (this.points[p2].y - this.points[p1].y) * (this.points[p3].x - this.points[p1].x);
