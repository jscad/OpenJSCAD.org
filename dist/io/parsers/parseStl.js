...
exports.parseBinarySTL = parseBinarySTL;
exports.parseAsciiSTL = parseAsciiSTL;
...
function parseBinarySTL(stl, fn) {
   ...
   for (var tr = 0; tr < totalTriangles; tr++) {
      ...
      if (skip == 0) {
         ...
         var w1 = new _csg.CSG.Vector3D(v1);
         var w2 = new _csg.CSG.Vector3D(v2);
         var w3 = new _csg.CSG.Vector3D(v3);
         var e1 = w2.minus(w1);
         var e2 = w3.minus(w1);
         var t = new _csg.CSG.Vector3D(no).dot(e1.cross(e2));
         if (t > 0) {
            // 1,2,3 -> 3,2,1
            var tmp = v3;
            v3 = v1;
   ...
   var src = "";
   src += "// producer: OpenJSCAD Compatibility (" + _version.version + ") STL Binary Importer\n";
   src += "// date: " + new Date() + "\n";
   src += "// source: " + fn + "\n";
   src += "\n";
   ...
   src += (0, _utils.vt2jscad)(vertices, triangles, normals, colors);
   src += "; }";
   return src;
}
...
function parseAsciiSTL(stl, fn) {
   ...
   src += "// producer: OpenJSCAD Compatibility (" + _version.version + ") STL ASCII Importer\n";
   src += "// date: " + new Date() + "\n";
   src += "// source: " + fn + "\n";
   src += "\n";
   ...
   for (o = 1; o < objects.length; o++) {
      ...
      src += (0, _utils.vt2jscad)(vertices, triangles, normals);
   }
   src += "); }\n";
   return src;
}
