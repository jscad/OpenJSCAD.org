...
var _debug = require('../../modeling/debug');
...
function parseAMF(src, fn, options) {
  ...
  var code = '';
  code += '//\n';
  code += "// producer: OpenJSCAD.org " + _version.version + " AMF Importer\n";
  code += "// date: " + new Date() + "\n";
  code += "// source: " + fn + "\n";
  code += '//\n';
...
function _old_parseAMF(amf, fn) {
  ...
  try {
    ...
    (0, _debug.echo)("XML parsing error:", e.message.substring(0, 120) + "..");
    ...
    mesh.each(function () {
      ...
      volume.each(function () {
        ...
        triangle.each(function () {
          ...
          if (rgbm.length || rgbv.length || rgbt.length) c[f.length] = rgbt.length ? rgbt : rgbv.length ? rgbv : rgbm;
  ...
  src += "// producer: OpenJSCAD Compatibility (" + _version.version + ") AMF Importer\n";
  src += "// date: " + new Date() + "\n";
  src += "// source: " + fn + "\n";
  src += "\n";
