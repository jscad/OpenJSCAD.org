    ...
    if (e.data instanceof Object) {
      ...
      if ('filename' in data) {
        ...
        if ('source' in data) {
          ...
          switch (e) {
            case 'amf':
              var parseAMF = require('./parsers/parseAMF').parseAMF;
              r.source = r.converted = parseAMF(data.source, data.filename);
              break;
            case 'gcode':
              var parseGCode = require('./parsers/parseGCode').parseGCode;
              r.source = r.converted = parseGCode(data.source, data.filename);
              break;
            case 'obj':
              var parseOBJ = require('./parsers/parseObj').parseOBJ;
            ...
            case 'scad':
              ...
              break;
            case 'stl':
              var parseSTL = require('./parsers/parseStl').parseSTL;
            ...
            case 'jscad':
              ...
              break;
            case 'svg':
              var parseSVG = require('./parsers/parseSVG').parseSVG;
              r.source = r.converted = parseSVG(data.source, data.filename);
              break;
            case 'json':
              var parseJSON = require('./parsers/parseJSON').parseJSON;
