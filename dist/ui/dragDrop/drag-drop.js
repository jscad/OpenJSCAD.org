...
exports.toggleAutoReload = toggleAutoReload;
exports.superviseAllFiles = superviseAllFiles;

var _conversionFormats = require('../jscad/conversionFormats');

var _createConversionWorker = require('../io/createConversionWorker');

var _createConversionWorker2 = _interopRequireDefault(_createConversionWorker);
...
function toggleAutoReload(toggle) {
  // console.log("toggleAutoReload()")
  if (toggle) {
    state.autoReloadTimer = setInterval(function () {
      superviseAllFiles();
    }, 1000);
  } else {
...
function superviseAllFiles(params, _ref) {
  var me = _ref.me,
      gMemFsCount = _ref.gMemFsCount,
      gMemFsTotal = _ref.gMemFsTotal,
      gMemFsChanged = _ref.gMemFsChanged,
      gRootFs = _ref.gRootFs,
      gCurrentFiles = _ref.gCurrentFiles;
  ...
  if (!gRootFs || gRootFs.length === 0 || me === 'web-offline') {
    // walkFileTree won't work with file:// (regardless of chrome|firefox)
    ...
    for (var _i = 0; _i < gRootFs.length; _i++) {
      walkFileTree(gRootFs[_i]);
    }
  }
}
...
function walkFileTree(item, path) {
  ...
  if (item.isFile) {
    ...
    item.file(function (file) {
      // this is also asynchronous ... (making everything complicate)
      var e = file.name.toLowerCase().match(/\.(\w+)$/i);
      e = RegExp.$1;
      if (_conversionFormats.conversionFormats.indexOf(e) >= 0) {
        state.gMemFsTotal++;
...
function loadLocalFiles(items, _ref2) {
  var gMemFs = _ref2.gMemFs,
      gMemFsCount = _ref2.gMemFsCount,
      gMemFsTotal = _ref2.gMemFsTotal,
      gMemFsChanged = _ref2.gMemFsChanged;
  ...
  for (var i = 0; i < items.length; i++) {
    var file = items[i];
    // console.log(file)
    var params = { gMemFs: gMemFs, gMemFsCount: gMemFsCount, gMemFsTotal: gMemFsTotal, gMemFsChanged: gMemFsChanged };
    var onSuccess = setCurrentFile;
...
function readFileAsync(file, _ref3, onError, onSuccess) {
  var gMemFs = _ref3.gMemFs,
      gMemFsCount = _ref3.gMemFsCount,
      gMemFsTotal = _ref3.gMemFsTotal,
      gMemFsChanged = _ref3.gMemFsChanged;
  ...
  reader.onloadend = function (evt) {
    if (evt.target.readyState === FileReader.DONE) {
      ...
      if (!gMemFs[file.name] || gMemFs[file.name].source !== source) gMemFsChanged++;

      saveScript(gMemFs, file.name, source);

      if (gMemFsCount === gMemFsTotal) {
        // -- are we done reading all?
        // console.log("readFileAsync: "+gMemFsTotal+" files read")
        var gMainFile = findMainFile(gMemFsTotal, gMemFs, file);
...
function updateDropZone(file, memFsTotal) {
  if (file) {
    var txt = 'Current file: ' + file.name;
    if (memFsTotal > 1) {
